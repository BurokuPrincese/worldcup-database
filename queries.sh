...
echo "$($PSQL "SELECT name FROM teams RIGHT JOIN games ON teams.team_id=games.winner_id WHERE round= 'Eighth-Final' AND year = 2014 UNION SELECT name FROM teams RIGHT JOIN games ON teams.team_id=games.opponent_id WHERE year = 2014 AND round ='Eighth-Final' ORDER BY NAME")"
